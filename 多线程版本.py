...
def main():
    start = time.time()
    # 进入首页 找个各个类别的页面链接 这里单线程就可以
    page_url_list = get_page_url()
    # 线程池里边 每个线程负责每个类别一页的信息提取和下载
    with ThreadPoolExecutor(16) as t:
        for page_url in page_url_list:
            for i in range(first_page, page_num + first_page):
                ...
                t.submit(multi_thread_1, page_url_new)
    end = time.time()
    print(f"共花费时间{end - start}秒")


def multi_thread_1(page_url):
    page_dict = get_img_page_info(page_url)
...
if __name__ == '__main__':
    # 爬取2个类别 2页 共花费时间244.17703127861023秒
